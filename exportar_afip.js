...
async function exportar_afip(empresa, desde, hasta) {
  let connection;
...
FROM (
    ...
    AND d.d2_cod= TRIM(CT.CPTOFACTURAERPID)
    INNER JOIN SB1010 B
...
const nombreArchivo = `DEBITO_FISCAL_${empresa}_${desde}_${hasta}.csv`;
...
csvStream.write({
  ...
  DEBITO_FISCAL: "DEBITO FISCAL FACTURADO",
  DEBITO_FISCAL_ODP: "DEBITO FISCAL O.D.P.",
...
for (const fila of result.rows) {
  csvStream.write({
    ...
    DEBITO_FISCAL_ODP: fila.DEBITOFISCAL === 0 ? "" : formatoAFIP(fila.DEBITOFISCAL),
...
exportar_afip(3, "20251101", "20251130");
